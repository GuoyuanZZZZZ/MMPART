...
def render_single_view(pc, view, device, background_color=(1,1,1), resolution=800, camera_distance=2.2, 
    ...
    return img, pc_idx, screen_coords, zbuf

def render_pc(xyz, rgb, save_dir, device, dir_name=None, save_depth=False):
    ...
    depth_dir = os.path.join(save_dir, "rendered_depth")
    if save_depth:
        os.makedirs(depth_dir, exist_ok=True)

    for i, view in enumerate(views):
        img, pc_idx, screen_coords, zbuf = render_single_view(pc, view, device)
        ...
        if save_depth:
            ...
            if np.any(valid):
                d = depth[valid]
                depth_norm[valid] = (d.max() - d) / max(d.max() - d.min(), 1e-8)
            plt.imsave(os.path.join(depth_dir, f"{i}.png"), depth_norm, cmap="gray")
